...
function Icons() {
  ...
  const [items,setItems] = useState(data.filter(item => item.category === 'icon')); // State to manage the selected pricing type
  const filter_data_according_to_pricing = (pricing_type) => {
    if(pricing_type === 'free')
      return setItems(data.filter(item => item.pricing === 'free' && item.category === 'icon'))
    else if(pricing_type === 'paid')
      return setItems(data.filter(item => item.pricing === 'paid' && item.category === 'icon'))
    else
      return setItems(data.filter(item => item.pricing === 'free,paid' && item.category === 'icon'))
  }
  ...
  return (
    <>
      ...
      <Meta name="description" content="diverse collection of icon libraries to help you convey ideas clearly and improve user experience with visual precision." />
      ...
      (
        <>
          ...
          <div className="flex flex-col items-start justify-center h-auto w-full sm:w-auto p-[5px]">
            <span className="font-semibold text-[32px] md:text-[32px] lg:text-[40px] xl:text-[50px] 2xl:text-[60px] font-Outfit mt-[90px] md:mt-[40px] lg:mt-[50px] xl:mt-[50px] 2xl:mt-[50px]">Icons</span>
